...
Unter dieser Url kommt dann die Standard Keycloak Login Page. Nach erfolgreicher Anmeldung bekommt der User seinen Token, wird automatisch wieder zur vorher besuchten Seite umgeleitet und es werden alle Funktionen der Webseite freigeschalten.
...
\end{lstlisting}
Die wie oben erwähnt zurzeit einzige Rolle \texttt{admin} hat kurz gesagt Rechte auf alles. Durch die Zeile \texttt{" clientRole " : false} is sie zum Beispiel auch nicht auf einen Gewissen Client beschränk sondern hat im ganzen Realm auf alles Zugriff.
...
\section{Angular}
Da da Projekt nicht nur am Desktop bedient werden kann ist die ganze Applikation responsive gemacht. 
\setauthor{Elias Mahr}
...
\begin{figure}[H]
    ...
    \caption{Kalender Element}
    \label{fig:CalenderEditor}
\end{figure}
Die \texttt{Calender} Componente erscheint wenn man in der \text{start-list-component} kein Baustelle sondern das blaue \text{Kalender} 
Element antippt. Als erstes sieht man sofort den Kalender, bei dem immer das aktuelle Monat ausgewählt ist.
...
Einmal Betriebsurlaub: Gelb markiert und ein Feiertag: Grau markiert. 
An diesen markierten Tagen werden die Container nicht eingeschalten.
Unter dem Kalender findet man dann noch eine Liste mit allen Urlauben für das Aktuelle Jahr und den Betriebsurlauben.
Die Feiertage werden von einer API geladen. Nach erfolgreicher Anmeldung wird der ``bearbeiten'' Button freigeschaltet und sie können Umgeändert werden.
...
Bei Klick auf de Butten kann dann der Betriebsurlaub simpel bearbeitet werden.
...
Um dies zu erleichtern wird beim eingeben eines neuen Datums sofort ein kleiner Kalender geöffnet auf dem ein neues Datum ausgewählt werden kann. Dies ist einer vorgefertigte Funktion von html wenn man die input type date verwendet.
...
Wenn man auf den Button \texttt{Dashboard} in der \texttt{start-list-componente} bei einem Container Tippt kommt man zur \texttt{dashboard} Komponente. 
Diese Hauptfunktion der Applikation ist für jeden Container individuell von den jeweiligen Daten.
Der Aufbau ist jedoch überall gleich. Die Componente ist so gestaltet das sie jederzeit erweitert werden kann. 
...
Ganz oben in der Componente sieht man die Überschrift mit dem Namen des Containers. Der erste Abschnitt ist Direkt eine übersichtliche Temperatur anzeige von Innen- und Außensensor.
Fett ist die Temperatur da das die wichtigste Anzeige im Container ist. Darunter gleich noch in ein wenig kleiner die Luftfeuchtigkeit und Abschließend noch die Anzeige der restlichen Batterie, um rechtzeitig zu erkenne wann die Batterien gewechselt werden sollten.
Ein Detailierter verlauf kann dann noch mit dem Button ``Verlauf'' angezeigt werden. Dazu aber dann in dieser Componente mehr.
